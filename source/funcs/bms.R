...
bms <-function(X.data,burn=1000,iter=NA,nmodel=500,mcmc="bd",g="UIP",mprior="random",mprior.size=NA,user.int=TRUE,
  ...
  if (is.function(mcmc)) { 
    ...
    if (length(grep("enum",mcmc,ignore.case=TRUE)))  {
      ...
      samplingfun=switch(int+1,BMS:::.fls.samp,BMS:::.fls.samp.int)
    } else {
  ...
  sampling=BMS:::.fixedset.sampler(samplingfun,fullK=K,fixed.pos=fixed.pos, X.data=X.data); 
  ######################################################################################################################################    
  ...
  pmplist=BMS:::.choose.mprior(mprior,mprior.size,K=K,X.data=X.data,fixed.pos=fixed.pos)
  mprior=pmplist$mp.mode;
  
  #g-prior
  gprior.info = BMS:::.choose.gprior(g,N,K,return.g.stats=g.stats,yty=yty,X.data=X.data) # gprior.info is a list that summarizes info about the choice of the g-prior
  ...
  start.list=BMS:::.starter(K,start.value,y,N=N,XtX.big=XtX.big,Xty.big=Xty.big,X=X,fixed.pos=fixed.pos)
  ...
  ols.object=BMS:::.ols.terms2(positions=(1:K)[molddraw==1],yty=yty,k=kold,N,K=K,XtX.big=XtX.big,Xty.big=Xty.big) #OLS results from starter model
  ...
  if (dotop) topmods=BMS:::.topmod.as.bbetaT(topmods,gprior.info,X.data)
  ...
  result=list(info=post.inf$info,arguments=BMS:::.construct.arglist(bms, environment()),topmod=topmods,start.pos=sort(start.position),gprior.info=post.inf$gprior.info,mprior.info=pmplist,reg.names=post.inf$reg.names,bms.call=try(match.call(bms,sys.call(0)),silent=TRUE))
